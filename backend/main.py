...
async def ip_lookup(query: IPLookup):
    ...
    try:
        # Use free IP API
        r = requests.get(
            f"http://ip-api.com/json/{ip}?fields=status,country,countryCode,region,regionName,city,zip,lat,lon,isp,org,as",
            timeout=5,
        )
    ...
    return {"ip": ip, "geo": geo_data, "timestamp": datetime.now().isoformat()}
...
async def ping_host(request: PingRequest):
    ...
    try:
        ...
        return {
            ...
            "stats": {
                "avg": round(duration, 2),
                "min": round(duration, 2),
                "max": round(duration, 2),
                "loss": "0%",
            },
            "timestamp": datetime.now().isoformat(),
        }
...
async def port_scan(request: PortScan):
    ...
    return {"host": host, "ports": results, "timestamp": datetime.now().isoformat()}
...
async def ssl_check(request: SSLCheck):
    ...
    try:
        ...
        with socket.create_connection(
            (request.domain, request.port), timeout=10
        ) as sock:
...
async def service_action(data: dict):
    ...
    try:
        if sys.platform == "win32":
            ...
            result = subprocess.run(
                ["sudo", "systemctl", action, service], capture_output=True, text=True
            )
...
async def view_logs(data: dict):
    ...
    try:
        if sys.platform == "win32":
            ...
            result = subprocess.run(
                ["journalctl", "-n", "20", "--no-pager"], capture_output=True, text=True
            )
...
async def view_auth_logs():
    ...
    try:
        if sys.platform == "win32":
            result = subprocess.run(
                ["powershell", "Get-EventLog", "-LogName", "Security", "-Newest", "20"],
                capture_output=True,
                text=True,
                shell=True,
            )
...
async def scan_network(data: dict):
    ...
    for i in range(start, min(end + 1, 255)):
        ...
        try:
            ...
            result = subprocess.run(
                ["ping", "-c", "1", "-W", "1", ip], capture_output=True, timeout=1
            )
            if result.returncode == 0:
                online.append(ip)
        except:
...
async def docker_logs(data: dict):
    """Get container logs"""
    import subprocess
...
async def wifi_list():
    ...
    try:
        if sys.platform == "darwin":
            ...
            current = (
                result2.stdout.strip() if result2.returncode == 0 else "Not connected"
            )
